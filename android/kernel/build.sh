...
TOOLCHAIN_CLANG_PATH=$TOOLCHAIN_PATH/clang-r487747c/bin
TOOLCHAIN_ARM64_PATH=$TOOLCHAIN_PATH/arm-gnu-toolchain-15.3.rel1-x86_64-aarch64-none-linux-gnu/bin
TOOLCHAIN_ARM_PATH=$TOOLCHAIN_PATH/arm-gnu-toolchain-15.3.rel1-x86_64-arm-none-linux-gnueabihf/bin
...
export PATH="$TOOLCHAIN_CLANG_PATH:$TOOLCHAIN_ARM64_PATH:$TOOLCHAIN_ARM_PATH:$PATH"

if ! command -v aarch64-none-linux-gnu-ld >/dev/null 2>&1; then
    echo "[aarch64-none-linux-gnu-ld] does not exist, please check your environment."
    exit 1
fi

if ! command -v arm-none-linux-gnueabihf-ld >/dev/null 2>&1; then
    echo "[arm-none-linux-gnueabihf-ld] does not exist, please check your environment."
    exit 1
fi
...
export CCACHE_DIR="$PWD/.cache/ccache_mikernel" 
...
MAKE_ARGS="ARCH=arm64 SUBARCH=arm64 O=out CC=clang CROSS_COMPILE=aarch64-none-linux-gnu- CROSS_COMPILE_ARM32=arm-none-linux-gnueabihf- CROSS_COMPILE_COMPAT=arm-none-linux-gnueabihf- CLANG_TRIPLE=aarch64-none-linux-gnu-"
...
echo "Done. The flashable zip is: [./output/$ZIP_FILENAME]"
